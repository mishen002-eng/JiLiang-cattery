...
export default function GalleryPage() {
  ...
  return (
    <section className="py-8 md:py-16 lg:py-24 bg-brand-cream">
      ...
      {/* Mobile: Instagram-style full-width 3-col grid */}
      <div className="mt-6 grid grid-cols-3 gap-0.5 md:hidden">
        {filtered.map((image, i) => (
          ...
          >
            <Image src={image.src} alt={image.alt} fill className="object-cover transition-transform hover:scale-[1.03]" />
          </div>
        ))}
      </div>

      {/* Desktop: masonry grid with captions */}
      <div className="container-wide">
        <div className="mt-12 hidden columns-3 gap-4 lg:columns-4 md:block">
          {filtered.map((image, i) => (
            <div
              key={`d-${i}`}
      ...
      <Dialog open={lightboxIndex !== null} onOpenChange={() => closeLightbox()}>
        ...
        >
          <DialogTitle className="sr-only">{currentImage?.caption || "Gallery image"}</DialogTitle>
          {currentImage && (
            <div className="flex w-full flex-col items-center justify-center" onClick={() => closeLightbox()}>
